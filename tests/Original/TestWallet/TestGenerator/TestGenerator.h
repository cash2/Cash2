...
public:
  ...
  test_generator(const CryptoNote::Currency& currency)
    : m_currency(currency) {
  }
  ...
  void addBlock(const CryptoNote::BlockTemplate& blk, size_t tsxSize, uint64_t fee, std::vector<size_t>& blockSizes,
    uint64_t alreadyGeneratedCoins);
  ...
  bool constructBlockManually(CryptoNote::BlockTemplate& blk, const CryptoNote::BlockTemplate& prevBlock,
    const CryptoNote::AccountBase& minerAcc, int actualParams = bf_none, uint64_t timestamp = 0, const Crypto::Hash& previousBlockHash = Crypto::Hash(),
