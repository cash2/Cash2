...
#include "crypto-ops.h"
        ...
        std::tuple<bool, std::vector<Crypto::Signature>> restore_ring_signatures(
            const Crypto::EllipticCurveScalar &derivationScalar,
        ...
        {
            std::vector<Crypto::Signature> restoredSignatures;
            ...
            {
                return {false, restoredSignatures};
            }
            ...
            /* We are now safe to copy the supplied signatures into the array and do the
               math to complete the restoration of the full ring signatures */
            std::copy(signatures.begin(), signatures.end(), std::back_inserter(restoredSignatures));
            ...
            sc_sub(
                reinterpret_cast<unsigned char *>(&restoredSignatures[realOutput]) + 32,
                reinterpret_cast<const unsigned char *>(&k),
                reinterpret_cast<const unsigned char *>(&derivedSecretKey));

            return {true, restoredSignatures};
        }
